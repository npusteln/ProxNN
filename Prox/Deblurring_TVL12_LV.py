...
"""
Reconstruction TV-isotropic with Loris-Verhoeven algorithm
----------------------------------------------------------
"""
...
param_tau      = 1.9/Anorm2
param_gamma    = 0.9/(param_tau*Lnorm2)
param_rho      = min(1,2-param_tau*Anorm2/2)
...
for k in range(param_iter):
  ...
  x_prev = xk.clone()
  tmp = vk+param_gamma*L(xk - param_tau*data_fidelity.grad(xk, y, physics) - param_tau*L_adjoint(vk))
  vk2 = tmp - param_gamma*prior.prox(tmp/param_gamma,gamma=param_regul/param_gamma)
  xk = xk - param_rho*param_tau*(data_fidelity.grad(xk, y, physics) + L_adjoint(vk2)) 
  vk = vk + param_rho*(vk2-vk)
